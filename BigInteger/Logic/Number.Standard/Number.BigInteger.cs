...
namespace Kzrnm.Numerics.Logic
{
    ...
    static partial class Number
    {
        ...
        internal static ParsingStatus TryParseBigIntegerHexOrBinaryNumberStyle<T>(ReadOnlySpan<T> value, NumberStyles style, out BigInteger result)
            where T : unmanaged
        {
            ...
            {
                if (!TryParseUnalignedBlock(value[0..DigitsPerBlock], out leading))
                {
                    goto FailExit;
                }
            ...
            int wholeBlockCount = value.Length / DigitsPerBlock;
            ...
            if (!TryParseWholeBlocks(value, wholeBlockDestination))
            {
                goto FailExit;
            }
        ...
        static uint GetSignBitsIfValid(uint ch) => (uint)((ch & 0b_1111_1000) == 0b_0011_0000 ? 0 : -1);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static bool TryParseUnalignedBlock<T>(ReadOnlySpan<T> input, out uint result)
            where T : unmanaged
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static bool TryParseWholeBlocks<T>(ReadOnlySpan<T> input, Span<uint> destination)
            where T : unmanaged
        {
            if (typeof(T) == typeof(char))
            {
        ...
        {
            int digits = 0;
            char fmt = ParseFormatSpecifier(format.AsSpan(), out digits);
            if (fmt == 'x' || fmt == 'X')
            {
            ...
            Debug.Assert(BigInteger.MaxLength * digitRatio + 1 < 0X7FFFFFC7); // won't overflow
            ...
            if (fmt == 'g' || fmt == 'G' || fmt == 'd' || fmt == 'D' || fmt == 'r' || fmt == 'R')
            {
                ...
                sNegative?.AsSpan().CopyTo(numberBuffer);
                BigIntegerToDecChars(numberBuffer, base1E9Value, digits);

                strResult = numberBuffer.ToString();
            ...
            else
            {
                ...
                {
                    NumberToStringFormat(ref vlb, ref number, format.AsSpan(), info);
                }
