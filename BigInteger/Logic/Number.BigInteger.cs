    ...
    internal static partial class Number
    {
        ...
        private static ParsingStatus NumberToBigInteger(ref NumberBuffer number, out BigInteger result)
        {
            ...
            static ParsingStatus DivideAndConquer(ref NumberBuffer number, out BigInteger result)
            {
                ...
                scoped Span<uint> currentBuffer;
                ...
                try
                {
                    ...
                    while (trailingZeroE9 != 0)
                    {
                        ...
                        multiplierSize <<= 1;
                        arrayFromPoolForMultiplier = ArrayPool<uint>.Shared.Rent(multiplierSize);
                        Span<uint> newMultiplier = new Span<uint>(arrayFromPoolForMultiplier, 0, multiplierSize);
                        newMultiplier.Clear();
                        BigIntegerCalculator.Square(multiplier, newMultiplier);
                        multiplier = newMultiplier;
                            ...
                            if (carry != 0)
                            {
                                if (buffer.Length < ++currentBufferSize)
                                {
                                    ...
                                    currentBuffer = (currentBufferSize <= BigIntegerCalculator.StackAllocThreshold
                                       ? stackalloc uint[BigIntegerCalculator.StackAllocThreshold]
                                       : resultBufferFromPool = ArrayPool<uint>.Shared.Rent(currentBufferSize)).Slice(0, currentBufferSize);
