    ...
    static partial class BigIntegerCalculator
    {
        ...
        private readonly ref struct Toom3Data
        {
                ...
                int cm2Sign)
            {
                ...
                this.cm1 = cm1;
                this.cm2 = cm2;
                this.cm1Sign = cm1Sign;
                this.cm2Sign = cm2Sign;
            }

            private readonly ReadOnlySpan<uint> c0;
            private readonly ReadOnlySpan<uint> c1;
            private readonly ReadOnlySpan<uint> cInf;
            private readonly ReadOnlySpan<uint> cm1;
            private readonly ReadOnlySpan<uint> cm2;
            private readonly int cm1Sign;
            private readonly int cm2Sign;
#endif
        ...
        static void MultiplySelfTwo(Span<uint> bits)
        {
            Debug.Assert(bits.IsEmpty || bits[^1] * 2ul < Base);
        ...
        static void DivideSelfTwo(Span<uint> bits)
        {
            Debug.Assert(bits.IsEmpty || bits[0] % 2 == 0);

            uint carry = 0;
            for (int i = bits.Length - 1; i >= 0; i--)
            {
                (bits[i], carry) = ((carry != 0 ? (Base / 2) : 0) + (bits[i] >> 1), bits[i] & 1);
            }
        }

        static void DivideThreeSelf(Span<uint> bits)
        {
            const uint oneThird = 1 * Base / 3;
            const uint twoThirds = 2 * Base / 3;
        ...
        static void Minus(Span<uint> bits)
        {
            int i = bits.IndexOfAnyExcept(0u);
            if (i < 0) return;
            bits[i] = Base - bits[i];

            for (++i; i < bits.Length; i++)
            {
                bits[i] = (Base - 1) - bits[i];
            }
        }
    }
}
